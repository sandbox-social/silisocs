...
```bash
uv run silisocs --config-path worlds/resource_market/conf world=resource_market agents=resource_market env=resource_market num_steps=6
```

For a no-LLM smoke run:

```bash
uv run silisocs --config-path worlds/resource_market/conf world=resource_market agents=resource_market env=resource_market sim.llm.provider=scripted sim.llm.name=scripted num_steps=1
```
