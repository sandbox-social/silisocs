...
```bash
uv run silisocs --config-path worlds/virtual_space/conf world=virtual_space agents=virtual_space env=virtual_space num_steps=6
```

For a no-LLM smoke run:

```bash
uv run silisocs --config-path worlds/virtual_space/conf world=virtual_space agents=virtual_space env=virtual_space sim.llm.provider=scripted sim.llm.name=scripted num_steps=1
```
