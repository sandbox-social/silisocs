...
async function saveStudy(){if(!yamlEditor)return true;const response=await fetch(`/api/studies/${studyId}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({yaml:yamlEditor.value,fingerprint:studyFingerprint,baseline:studyBaseline})});const conflict=await saveConflict(response);if(conflict){showSaveConflict(conflict,{onReload:()=>{if(!confirm(`Reloading ${conflict.file} discards your unsaved edits to this study.`))return;studyDirty=false;location.reload()},onOverwrite:async()=>{studyFingerprint=conflict.fingerprint;await saveStudy()}});return false}if(response.ok){const saved=await response.json();studyFingerprint=saved.fingerprint||'';studyBaseline=yamlEditor.value;markStudySaved()}notify(response.ok?'Saved':await response.text(),response.ok?'success':'danger');return response.ok}
