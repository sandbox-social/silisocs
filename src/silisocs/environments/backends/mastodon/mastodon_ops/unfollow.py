...
def unfollow(login_user: str, unfollow_user: str) -> None:
    ...
    Raises
    ------
        AccountNotFoundError: If the account to unfollow is not found.
        ValueError: If there is a problem with the login or the unfollow call.
    ...
    try:
        ...
        if not account:
            raise AccountNotFoundError(f"User {unfollow_user} not found.")
