"""Event semantics: the vocabulary a backend uses to describe its own actions.

Open action tags and semantic payload fields, derived from a backend's
``@app_action`` catalog (plus an optional class-level ``event_semantics``
declaration) and consumed by the analysis surfaces through
``run_manifest.json``. This lives in the environments layer because backends
DECLARE their semantics: keeping the vocabulary here means a backend never has
to import the evaluation/analysis layers that read it, and the dependency stays
one-directional (evaluations/analysis -> environments).

``silisocs.evaluations.vocabulary`` re-exports this module's public seams
(``register_event_semantics``, ``EventSemantics``, ...) for readers above the
environments layer.
"""
