...
from silisocs.utils.memory import create_memory_bank
from silisocs.utils.misc import SimMetricsCollector
...
class Simulation(simulation_lib.Simulation):
    ...
    def __init__(
        ...
        memory_backend: str = "associative",
    ):
        ...
        Args:
          ...
          models: the language models to use.
          entity_to_model: mapping from entity names to model names.
          embedder: the sentence transformer to use. Can be None for list memory.
          engine: the engine to use, defaults to sequential.Sequential().
        """
        self._config: prefab_lib.Config = config
        self._models = models
        ...
        self.game_master_memory_bank = create_memory_bank(
            backend=self._memory_backend,
            sentence_embedder=embedder,
        )
        all_data = self._config.instances
        gm_configs = [entity_cfg for entity_cfg in all_data if entity_cfg.role == Role.GAME_MASTER]
        entities_configs = [entity_cfg for entity_cfg in all_data if entity_cfg.role == Role.ENTITY]
        initializer_configs = [
            entity_cfg for entity_cfg in all_data if entity_cfg.role == Role.INITIALIZER
        ]
        ...
        for entity_config in entities_configs:
            ...
            name = entity_config.params.get("name", "unknown")
            print(f"Built entity '{name}' in {elapsed:.2f}s")

        for gm_config in initializer_configs + gm_configs:
            t0 = time.time()
            self.add_game_master(gm_config)
            elapsed = time.time() - t0
            name = gm_config.params.get("name", "unknown")
            print(f"Built game master '{name}' in {elapsed:.2f}s")
    ...
    def add_game_master(
        ...
        game_master_prefab = copy.deepcopy(self._config.prefabs[instance_config.prefab])
    ...
    def play(
        ...
        with metrics.phase("html_log_generation"):
            player_logs = []
            player_log_names = []
            ...
            game_master_memories = self.game_master_memory_bank.get_all_memories_as_text()
    ...
    def make_checkpoint_data(self) -> dict[str, Any]:
        ...
        for entity in self.entities:
            ...
            entities_data[entity.name] = save_data
        ...
        for gm in self.game_masters:
            ...
            game_masters_data[gm.name] = save_data
    ...
    def load_from_checkpoint(
        ...
        for gm_name, state in gm_states.items():
            ...
            try:
                role = (
                    prefab_lib.Role[role_name]
                    if role_name in prefab_lib.Role.__members__
                    else prefab_lib.Role.GAME_MASTER
                )
            except KeyError:
                print(f"Warning: Invalid role {role_name} for {gm_name}, using GAME_MASTER.")
    ...
    def _load_entity_from_state(
        ...
        if default_role == Role.ENTITY:
            existing_entity = next((e for e in self.entities if e.name == entity_name), None)
            if existing_entity:
            ...
            existing_gm = next((gm for gm in self.game_masters if gm.name == entity_name), None)
            if existing_gm:
