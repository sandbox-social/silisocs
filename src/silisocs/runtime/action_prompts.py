...
def prompt_additions_from_cfg(cfg: Any) -> PromptAdditions:
    ...
    if isinstance(value, bool):
        ...
        # Backward-compatible support for legacy nested schema:
        # sim.prompt_additions.action_count_guidance.add_to_prompt
        action_count_guidance = bool(value.get("add_to_prompt", True))
    else:
        action_count_guidance = True

    return PromptAdditions(add_action_count_guidance=action_count_guidance)
    ...
    *,
    ...
    if normalized_action_mode == "generic":
        ...
        if custom_prompt:
            ...
            base_prompt = str(getattr(_env_cfg(cfg), "action_prompt", "") or "").strip()
...
def build_action_prompt_with_app_instance(
    *,
    ...
    Tool-calling marker/schema injection intentionally does not happen here.
    It is performed by GM act component at runtime with the real app instance.
    """
    del sm_app
