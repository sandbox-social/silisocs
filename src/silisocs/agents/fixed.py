"""Fixed-action native agent with sequence support.

A lightweight agent type that does not call an LLM. It executes
...
This agent is useful for deterministic tests and scripted scenarios.
"""
...
class FixedAgent(Agent):
    """Runtime agent implementing observe/act with deterministic action sequences.
    ...
    def __init__(
        ...
        *,
        ...
        name: str = "FixedAgent",
        seed_post: str = "",
        fixed_action_plan: Mapping[Any, Any] | None = None,
        fixed_action_plan_file: str = "",
        allowed_action_types: list[str] | None = None,
        action_output_mode: str = "auto",
        advance_without_episode_observation: bool = True,
        advance_episode_on_non_episode_observation: bool = False,
        emit_finished_on_episode_end: bool = False,
        finished_action_name: str = "FINISHED",
        ...
        **extra_params: Any,
        ...
        self._allowed_action_types = [
            str(v).strip().upper() for v in (allowed_action_types or []) if str(v).strip()
        ]

        self._action_output_mode = str(action_output_mode or "auto").strip()
        self._advance_without_episode_observation = bool(advance_without_episode_observation)
        self._advance_episode_on_non_episode_observation = bool(
            advance_episode_on_non_episode_observation
        )
        self._emit_finished_on_episode_end = bool(emit_finished_on_episode_end)
        self._finished_action_name = str(finished_action_name or "FINISHED").strip() or "FINISHED"
        ...
        plan_file = str(fixed_action_plan_file or "").strip()
        plan = fixed_action_plan
        if plan_file:
            if plan not in (None, {}):
                raise ValueError(
    ...
    def _effective_action_output_mode(self, action_spec: Any) -> str:
        ...
        if getattr(action_spec, "output_type", None) == OutputType.TOOL_CALLS:
            return "tool_calling"
        action_prompt = str(getattr(action_spec, "prompt", "") or "")
        if "Respond with EXACTLY ONE action" in action_prompt or "ACTION:" in action_prompt:
