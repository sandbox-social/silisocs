# mastodon_sim/runtime/config.py
"""
...
def validate_cross_references(cfg: DictConfig) -> None:
    ...
    if hasattr(scenario_cfg, "probes"):
        ...
        if hasattr(probes_cfg, "queries_data"):
            queries_cfg = probes_cfg.queries_data
        elif hasattr(probes_cfg, "queries"):
            queries_cfg = probes_cfg.queries

        if queries_cfg is not None:
            for query_num, query_data in queries_cfg.items():
                if hasattr(query_data, "interaction_premise_template"):
                    premise = query_data.interaction_premise_template
                elif hasattr(query_data, "query_data") and hasattr(
                    query_data.query_data, "interaction_premise_template"
                ):
                    premise = query_data.query_data.interaction_premise_template
                else:
                    continue
