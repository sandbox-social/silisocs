...
- `sim/` is the multi-LLM-agent simulation library
- `scenarios/` has scenario specific configuration
- `mastodon_sim/` is the library of tools for running and deploying the (in this case Mastodon) social media server.

`mastodon-sim/src/sim/main.py` runs the simulation orchestrated in `config.yaml` located in `mastodon-sim/conf/`and generated by running `mastodon-sim/src/sim/config_utils/config_schema.py`

### Configuration
We provide an example application that simulates an election simulation as a specific scenario. Scenario-specific configuration code for this scenario is located in `mastodon-sim/sim/scenarios/election`.
