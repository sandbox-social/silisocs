...
configuration. It lives in `worlds/<name>/` and can be reused across many studies.
...
type `/new-world` to be guided through this process interactively.
...
`source: hf_dataset` (a HuggingFace dataset). See `agent_docs/world_design.md`
...
With overrides:
```bash
uv run silisocs --config-path worlds/my_world/conf num_steps=20 seed=99
```
...
```bash
uv run silisocs --config-path worlds/my_world/conf num_steps=1 sim.llm.provider=scripted
```

Add a `worlds/my_world/README.md` so the world is discoverable (see
existing worlds like `worlds/neighborhood_forum/README.md` for the format).
...
`agent_docs/world_design.md` for the fixed-action schema.

### Switching backends

Create `worlds/my_world/conf/env.yaml` with a single line:
```yaml
gm:
  backend:
...
- **Existing worlds:** `worlds/election/`, `worlds/ai_conference/` — working examples
- **Full config reference:** `docs/configuration.md`
- **Study design:** `docs/study_guide.md` — how to run multi-condition experiments on your world
- **Probes (survey questions):** `docs/probes.md`
- **Advanced config:** `agent_docs/world_design.md` — complete field reference
