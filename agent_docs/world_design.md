...
This guide is for LLM agents helping design and create new simulation worlds using configuration (YAML) — without modifying code.

## 1) Scenario Design Workflow

A complete world consists of:
...
5. **Optional sim/backend overrides** — Customize sim parameters or backend type for this world

### Creating a New Scenario

**Directory Structure:**
```
worlds/{world_name}/conf/
├── world/
...
See `worlds/election/conf/world/default.yaml` for a production world with all sections.
...
| Inline YAML | `source: inline`<br>`records: [{name: Alice, bio: "..."}]` | Defined directly in world |
...
Create `worlds/{name}/conf/sim.yaml` to customize sim parameters (LLM, engine, tool-calling).
...
llm:
  name: gpt-4o                     # Override model for this world
...
**Important:** Missing fields automatically fall back to `src/silisocs/conf/sim/base.yaml` defaults. This keeps world files minimal.
...
Use `env.gm.backend.enabled_actions` to narrow a world to the action set you want.
...
Configure in world persona_pipeline:
...
2. **Your world overrides** (higher priority via SearchPath plugin)
   - `worlds/my_world/conf/world/default.yaml` (replaces package world group)
   - `worlds/my_world/conf/agents/default.yaml` (replaces package agents group)
   - `worlds/my_world/conf/sim.yaml` (merged into sim group, if present)
   - `worlds/my_world/conf/env.yaml` (merged into env group, if present)
...
- If world omits a probe, that probe is not deployed

This means your world files only need to specify what's **different** from defaults.
...
- **Check**: `worlds/{name}/conf/world/default.yaml` exists
...
1. **Study existing example**: review `worlds/election/conf/world/election.yaml`
2. **Understand defaults**: check `src/silisocs/conf/sim/base.yaml`
3. **Create your world**: mkdir and scaffold with the structure above
...
- Expand one declarative file into many concrete runs (world x condition x seed).
...
study:
  ...
  run_defaults:
    ...
    output_root_override: "experiments/studies/{study_id}/runs/{hypothesis_id}/{condition_id}/{world}/seed_{world}/run"
    seed_start: 11
    seed_repeats: 3
    overrides:
      num_agents: 50
...
| `study.worlds` | list[string] | recommended | Base world loop for all conditions. |
...
experiments/studies/{study_id}/generated/
  ...
  study_enriched.yaml
  logs/{run_id}.log
  eval/{hypothesis}/{condition}/{world}/seed_{seed}/{eval_id}/*.json
```

Simulation outputs are organized by study/hypothesis/condition/world with seed at the lowest level:

```text
experiments/studies/{study_id}/runs/{hypothesis_id}/{condition_id}/{world}/seed_{world}/run
```
...
- World-local `worlds/<name>/outputs/...`: reserve for ad hoc/manual world testing outside study orchestration.
