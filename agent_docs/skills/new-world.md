...
A world is a **shared social world**: a setting, a cast of agents, and a backend
configuration. It lives in `worlds/<name>/` and can be used as the substrate for
...
Ask:
> "Are you designing this world for an existing study or research question, or
...
- After confirming the event (Step 1), add a brief note:
  > "Signal check: here's why this world should produce meaningful variation
...
Ask:
> "What social phenomenon does this world capture? Describe it in a sentence or two —
...
sentences): why this world would produce variation relevant to the research
...
| **ConcordiaAgent** | `silisocs.agents.concordia.ConcordiaAgent` with `compat: concordia` | Explicit compatibility for older Concordia-style world agents |
...
- If it requires capabilities none of these classes has (e.g. multi-agent coordination,
  custom tools, access to external data at runtime, components that carry world-specific semantics): say so clearly.
...
- `env.gm.backend.enabled_actions`: leave as `null` for the full backend action
  surface, or list the exact actions this world should allow. For a small
...
Ask:
> "What should this world be called? (short, snake_case slug — e.g. `vaccine_debate`)"
...
```bash
uv run silisocs new-world --from-spec-json '<JSON>'
```
...
```bash
uv run silisocs --config-path worlds/<name>/conf num_steps=1 sim.llm.provider=scripted
```
