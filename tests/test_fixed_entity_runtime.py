...
def test_fixed_advances_episode_without_observation() -> None:
    entity = FixedAgent(
        name="Fixed",
    ...
    assert "CONTENT: episode-0-a" in first.text
    assert "CONTENT: episode-0-b" in second.text
    assert "CONTENT: episode-1-a" in third.text


def test_fixed_non_episode_observation_advances_action_then_episode() -> None:
    entity = FixedAgent(
        name="Fixed",
        action_output_mode="parsed_action",
        advance_without_episode_observation=True,
...
def test_fixed_ignores_numeric_non_episode_observation() -> None:
    entity = FixedAgent(
        name="Fixed",
        action_output_mode="parsed_action",
        advance_without_episode_observation=True,
    ...
    assert "CONTENT: episode-0-a" in first.text
    assert "CONTENT: episode-1-a" in second.text


def test_fixed_uses_explicit_episode_marker_from_observation() -> None:
    entity = FixedAgent(
        name="Fixed",
        action_output_mode="parsed_action",
        advance_without_episode_observation=True,
    ...
    assert "CONTENT: episode-2-a" in result.text


def test_fixed_emits_tool_call_format_for_finished() -> None:
    entity = FixedAgent(
        name="Fixed",
    ...
    assert result.tool_calls[0].name == "FINISHED"
    assert result.tool_calls[0].arguments == {}


def test_fixed_can_emit_finished_at_episode_end() -> None:
    entity = FixedAgent(
        name="Fixed",
    ...
    assert "ACTION: create_tweet" in first.text
    assert "ACTION: FINISHED" in second.text


def test_fixed_does_not_cycle_after_exhaustion() -> None:
    entity = FixedAgent(
        name="Fixed",
        action_output_mode="parsed_action",
        advance_without_episode_observation=True,
    ...
    assert "CONTENT: episode-0-a" in first.text
    assert "CONTENT: episode-1-a" in second.text
    assert "ACTION TYPE: FINISHED" in third.text
    assert "ACTION TYPE: FINISHED" in fourth.text


def test_fixed_requires_dict_action_plan() -> None:
    with pytest.raises(ValueError, match="fixed_action_plan must be an episode-keyed dict"):
        FixedAgent(
            name="Fixed",
            fixed_action_plan=cast(
                Any,
                [{"action_type": "post", "content": "legacy-list"}],
            ),
        )


def test_fixed_loads_action_plan_from_json_file(tmp_path: Path) -> None:
