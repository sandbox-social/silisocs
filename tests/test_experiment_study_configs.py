...
def test_study_docs_use_current_schema_terms() -> None:
    ...
    for path in docs:
        text = path.read_text(encoding="utf-8")
        found = [term for term in removed if term in text]
        if found:
            offenders[str(path.relative_to(PROJECT_ROOT))] = found
...
def test_source_only_release_scan_ignores_historical_artifacts() -> None:
    """Historical generated outputs stay tracked, but live source must be current."""
    scanned_paths: list[Path] = []
    scanned_paths.extend((PROJECT_ROOT / "src").rglob("*.py"))
    scanned_paths.extend((PROJECT_ROOT / "scenarios").rglob("*.yaml"))
    scanned_paths.extend((PROJECT_ROOT / "scenarios").rglob("*.md"))
    scanned_paths.extend((PROJECT_ROOT / "replications").rglob("*.yaml"))
    scanned_paths.extend((PROJECT_ROOT / "replications").rglob("*.py"))
    scanned_paths.extend((PROJECT_ROOT / "docs").rglob("*.md"))
    scanned_paths.extend((PROJECT_ROOT / "agent_docs").rglob("*.md"))
    scanned_paths.extend((PROJECT_ROOT / "slurm_scripts").glob("*.sh"))
    scanned_paths.extend(_active_study_files())
    scanned_paths.append(PROJECT_ROOT / "experiments" / "run_study.py")

    ignored_parts = {"generated", "graph_experiments"}
    stale_terms = (
        "sim.seed=",
        "sim.run_name",
        "sim.num_steps",
        ...
        "env.platform_type",
        "env.app.",
        "slurm_scripts/narval",
        "slurm_scripts/tamia",
        "study-array-worker.sh",
        "runner-worker.sh",
        "experiments/scripts/organize_experiments.py",
    )
    offenders: dict[str, list[str]] = {}
    for path in sorted(set(scanned_paths)):
        ...
        found = [term for term in stale_terms if term in text]
        if found:
            offenders[str(path.relative_to(PROJECT_ROOT))] = found
